...
SELECT tier, 
    CONCAT(
            FLOOR(SUM(TIMESTAMPDIFF(HOUR, dt_start, dt_end)) / 24), ' days ',
            MOD(SUM(TIMESTAMPDIFF(HOUR, dt_start, dt_end)), 24), ' hours ',
            MOD(SUM(TIMESTAMPDIFF(MINUTE, dt_start, dt_end)), 60), ' minutes ',
